...
use rimeflow_yolov8n::postprocess::{decode_yolo_output, nms, Detection};
...
fn task1_raw_golden_uses_production_decode_and_nms() {
    ...
    let manifest: Value =
        serde_json::from_slice(&fs::read(root.join("evidence/fixtures/manifest.json")).unwrap())
            .unwrap();
    ...
    for entry in fixtures {
        ...
        let fixture: Value =
            serde_json::from_slice(&fs::read(root.join(entry["path"].as_str().unwrap())).unwrap())
                .unwrap();
