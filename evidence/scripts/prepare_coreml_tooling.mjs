...
const environment = resolve(root, process.env.RIMEFLOW_COREML_VENV ?? '.evidence/coreml/verify-venv');
...
run(pip, [
  ...
  '-r', resolve(root, 'evidence/tooling/coreml-requirements.lock'),
]);
run(pip, ['check']);
if (!output(pip, ['--version']).startsWith(expectedPip)) throw new Error(`安装后 pip 必须保持 ${expectedPip}`);
run(python, ['-c', [
  'import coremltools, torch, torchvision, ultralytics',
  'print(coremltools.__version__, torch.__version__, torchvision.__version__, ultralytics.__version__)',
].join('; ')]);
